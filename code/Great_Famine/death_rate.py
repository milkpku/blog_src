...
def plot_bar():
  ...
  data = [
    go.Scatter(x=year, y=birth_rate, mode="lines+markers", name="出生率"),
    go.Scatter(x=year, y=death_rate, mode="lines+markers", name="死亡率"),
  ]

  layout = go.Layout(
    title='1950-1970年中国出生/死亡率',
    yaxis=dict(
      title='出生/死亡率（‰）')
    )
  ...
  plotly.offline.plot(fig, filename='death_rate.html', include_plotlyjs=False)
