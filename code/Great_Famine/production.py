...
def plot_bar():
  ...
  data = [go.Bar(x=year, y=food_production, marker_color='forestgreen')]

  layout = go.Layout(
    title='1950-1970年中国粮食产量',
    yaxis=dict(
      title='年粮食产量（万吨）')
    )
  ...
  plotly.offline.plot(fig, filename='production.html', include_plotlyjs=False)
